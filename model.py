...
# Inputs: modelYear, mileage, horsepower
# Output: price
...
if __name__ == '__main__':
    ...
    filepath = 'app/used_cars.csv'  # Adjust this path to your dataset location
    X_train, y_train = load_and_preprocess_data(filepath)
    ...
    # User inputs for model year, mileage, and horsepower
    while (1 > 0):
        model_year_input = int(input("Enter the car's model year: "))
        mileage_input = int(input("Enter the car's mileage: "))
        horsepower_input = int(input("Enter the car's horsepower: "))

        # Convert model year to car age
        car_age = 2024 - model_year_input

        input_datapoint = [[car_age, mileage_input, horsepower_input]]  # Updated input for prediction
        prediction = tuned_RF.predict(input_datapoint)
